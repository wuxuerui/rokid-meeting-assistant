...
{
  "navigationBarTitleText": "Rotation Monitor"
}
</script>
...
function classifyRotation(magnitude) {
  if (typeof magnitude !== 'number' || !Number.isFinite(magnitude)) {
    return {
      rotationState: 'Waiting for rotation',
      stabilityHint: 'Start the sensor to inspect head-turn speed and short rotation bursts.',
    };
  }

  if (magnitude >= 2.5) {
    return {
      rotationState: 'Rapid sweep',
      stabilityHint: 'A fast head-turn or quick scan is in progress.',
    };
  }

  if (magnitude >= 1.2) {
    return {
      rotationState: 'Active scan',
      stabilityHint: 'Rotation is clearly visible and the device is not in a stable hold.',
    };
  }

  return {
    rotationState: 'Stable hold',
    stabilityHint: 'Angular speed looks calm and suitable for steady viewing.',
  };
}
...
export default {
  ...
  updateReadingSnapshot() {
    ...
    const magnitudeValue =
      [x, y, z].every((value) => typeof value === 'number' && Number.isFinite(value))
        ...
        : null;
    const rotation = classifyRotation(magnitudeValue);
    this.setData({
      ...
      rotationState: rotation.rotationState,
      stabilityHint: rotation.stabilityHint,
    });
  },
  ...
  bindSensor(sensor) {
    ...
    sensor.addEventListener('error', (event) => {
      this.updateReadingSnapshot();
      const message =
        event && event.message ? `${event.error || 'error'}: ${event.message}` : 'Unknown gyroscope error';
  ...
  createSensor(initial = false) {
    ...
    if (typeof Gyroscope === 'undefined') {
      ...
      this.log('Gyroscope is not available in this runtime');
      return;
    }

    try {
      const sensor = new Gyroscope({ frequency: this.data.frequency });
      this.sensor = sensor;
      ...
      this.clearError();
      this.log(initial ? 'Rotation monitor instance ready' : 'Created a fresh rotation monitor instance');
    } catch (error) {
      this.sensor = null;
  ...
  startSensor() {
    if (!this.sensor) {
      this.setError('Gyroscope is unavailable');
      return;
    }
    try {
      this.clearError();
  ...
  <view class="container">
    <view class="hero-card">
      ...
      <view class="hero-header">
        <text class="page-title">Rotation Stability Monitor</text>
        <text class="hero-chip">{{available ? 'READY' : 'UNAVAILABLE'}}</text>
      </view>
      <text class="subtitle">
        用角速度流判断快速转头、轻微扫描和稳定视线，更适合佩戴设备的现场姿态观察与转动稳定性检查。
      </text>
    </view>
    ...
    <view class="data-card">
      <view class="section-header">
        <text class="section-title">Rotation Snapshot</text>
      ...
      <view class="axis-grid">
        <view class="axis-card axis-x">
          ...
          <text class="axis-unit">rad/s</text>
        </view>
      ...
      <view class="reading-meta">
        <text class="meta-line">Rotation State: {{rotationState}}</text>
        <text class="meta-line">Stability Hint: {{stabilityHint}}</text>
        <text class="meta-line">Angular Speed: {{magnitude}} rad/s</text>
