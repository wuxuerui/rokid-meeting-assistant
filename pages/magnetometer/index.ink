...
{
  "navigationBarTitleText": "Field Check"
}
</script>
...
function classifyField(magnitude) {
  if (typeof magnitude !== 'number' || !Number.isFinite(magnitude)) {
    return {
      fieldState: 'Waiting for field data',
      calibrationHint: 'Start sampling to inspect whether the environment is suitable for heading use.',
    };
  }

  if (magnitude >= 70) {
    return {
      fieldState: 'Magnetic noise',
      calibrationHint: 'Field strength is unusually high and may indicate nearby interference.',
    };
  }

  if (magnitude <= 25) {
    return {
      fieldState: 'Weak field',
      calibrationHint: 'Magnetic response looks light and may need a cleaner calibration posture.',
    };
  }

  return {
    fieldState: 'Clean field',
    calibrationHint: 'Current field strength looks healthy for heading and direction checks.',
  };
}
...
export default {
  ...
  updateReadingSnapshot() {
    ...
    const magnitudeValue =
      [x, y, z].every((value) => typeof value === 'number' && Number.isFinite(value))
        ...
        : null;
    const field = classifyField(magnitudeValue);
    this.setData({
      ...
      calibrationHint: field.calibrationHint,
    });
  },
  ...
  bindSensor(sensor) {
    sensor.addEventListener('activate', () => {
      ...
      this.log('Magnetometer activated');
    });
  ...
  createSensor(initial = false) {
    ...
    if (typeof Magnetometer === 'undefined') {
      ...
      this.log('Magnetometer is not available in this runtime');
      return;
    }

    try {
      const sensor = new Magnetometer({ frequency: this.data.frequency });
      this.sensor = sensor;
      ...
      this.clearError();
      this.log(initial ? 'Field check instance ready' : 'Created a fresh field check instance');
    } catch (error) {
      this.sensor = null;
  ...
  startSensor() {
    if (!this.sensor) {
      this.setError('Magnetometer is unavailable');
      return;
    }
    try {
      this.clearError();
  ...
  <view class="container">
    <view class="hero-card">
      ...
      <view class="hero-header">
        <text class="page-title">Magnetic Environment Check</text>
        <text class="hero-chip">{{available ? 'READY' : 'UNAVAILABLE'}}</text>
      </view>
      <text class="subtitle">
        用磁场强度和三轴读数判断当前环境是否适合做方向感知与罗盘校准，而不是只看原始磁力计数值。
      </text>
    </view>
    ...
    <view class="data-card">
      <view class="section-header">
        <text class="section-title">Environment Snapshot</text>
      ...
      <view class="reading-meta">
        <text class="meta-line">Field State: {{fieldState}}</text>
        <text class="meta-line">Calibration Hint: {{calibrationHint}}</text>
        <text class="meta-line">Field Magnitude: {{magnitude}} uT</text>
    ...
    <view class="control-card">
      ...
      <view class="button-grid" role="navigation">
        <button class="btn" bindtap="startSensor">Start Check</button>
    ...
    <view class="log-card">
      <view class="section-header">
        <text class="section-title">Environment Record</text>
